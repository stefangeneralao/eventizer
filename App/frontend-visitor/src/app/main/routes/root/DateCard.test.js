...
describe('<DateCard />', () => {
  ...
  it('should contain ImageComponent-child', () => {
    const wrapper = shallow(
      <DateCard
        date={data.dates[0]['date']}
        events={data.dates[0]['events']}
    ...
    expect(wrapper.find(ImageComponent)).toHaveLength(1);
  });

  it('should contain EventContainer-child', () => {
    const wrapper = shallow(
      <DateCard
        date={data.dates[0]['date']}
        events={data.dates[0]['events']}
    ...
    expect(wrapper.find(EventContainer)).toHaveLength(1);
  });
});

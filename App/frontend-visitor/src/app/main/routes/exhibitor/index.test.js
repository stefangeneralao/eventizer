...
import Exhibitor from './index.js';
import ExhibitorCard from './ExhibitorCard';

// Testsuite for main/routes/exhibitor/index.js
...
describe('<Exhibitor />', function() {
  it('should have className Exhibitor', function() {
    const wrapper = shallow(
      <Exhibitor
      store={data}
      />
    );
    expect(wrapper.hasClass("Exhibitor"));
  });

  it('should contain ExhibitorCard-child', () => {
    const wrapper = shallow(
      <Exhibitor
        store={data}
        />
      );
    expect(wrapper.find(ExhibitorCard)).toHaveLength(1);
  });
});
